...
$dbPath = Join-Path $tmpDir "perf-baseline-mongo-db"
$logPath = Join-Path $tmpDir "perf-baseline-mongo.log"
...
try {
  ...
  $env:MONGODB_DB = "cbsp_perf_baseline_$([DateTimeOffset]::UtcNow.ToUnixTimeSeconds())"

  Push-Location $WorkspaceRoot
  try {
    & npm.cmd run perf:baseline:mongo
    if ($LASTEXITCODE -ne 0) {
      throw "perf:baseline:mongo failed with exit code $LASTEXITCODE"
    }
  } finally {
    Pop-Location
  }
